...
const adminBlackListModel = require("../models/adminBlackList")

adminRouter.post("/register",async(req,res)=>{
    const {email,password} = req.body
    try {
        const newPass = await bcrypt.hash(password,10)
        const adminuser = await AdminModel.create({...req.body,password:newPass})
...
adminRouter.post("/login",async(req,res)=>{
    ...
    try {
        ...
        if(!comparePass){
            ...
            const token = jwt.sign({userId:adminuser._id,name:adminuser.name},process.env.secretKey,{expiresIn:"1d"})
